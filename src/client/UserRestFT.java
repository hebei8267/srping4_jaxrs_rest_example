...
import webservice.rest.UserDTO;

import com.google.common.collect.Lists;

/**
 * 演示Rest Service的客户端，包含： 1. 使用exchange()函数与使用对Header操作的两种方式 2. JDK
 * HttpConnection与Apache HttpClient4两种底层 3. 验证JSON与XML两种编码 4.
 * 验证与Shiro结合的HttpBasic验证
 */
public class UserRestFT {
	private static String resourceUrl = "http://127.0.0.1/jaxrs_rest/rest/API/user/getUserInfo/";
	...
	public void initRestTemplate() {
		...
		ClientHttpRequestInterceptor interceptor = new HttpBasicInterceptor("bob", "bobbob");
	...
	/**
	 * 演示使用原始的exchange()方法来设置Headers. 演示xml格式数据. 演示jdk connection.
	 */
	@Test
	public void getUserAsXML() {
		// 设置Http Basic参数
		HttpHeaders requestHeaders = new HttpHeaders();
		requestHeaders.set(com.google.common.net.HttpHeaders.AUTHORIZATION, Servlets.encodeHttpBasic("bob", "bobbob"));
		System.out.println("Http header is" + requestHeaders);
		HttpEntity<?> requestEntity = new HttpEntity(requestHeaders);

		try {
			HttpEntity<UserDTO> response = jdkTemplate.exchange(resourceUrl + "/{id}.xml", HttpMethod.GET, requestEntity,
					UserDTO.class, 1L);

			// 直接取出XML串
			HttpEntity<String> xml = jdkTemplate.exchange(resourceUrl + "/{id}.xml", HttpMethod.GET, requestEntity, String.class,
					1L);
	 ...
	 * 演示json格式数据. 演示使用Apache Http client4.
	 */
	@Test
	...
	public void authWithHttpBasic() {
		// 设置Http Basic参数
		HttpHeaders requestHeaders = new HttpHeaders();
		requestHeaders.set(com.google.common.net.HttpHeaders.AUTHORIZATION, Servlets.encodeHttpBasic("admin", "wrongpassword"));
		HttpEntity<?> requestEntity = new HttpEntity(requestHeaders);

		try {
			jdkTemplate.exchange(resourceUrl + "/{id}.xml", HttpMethod.GET, requestEntity, UserDTO.class, 1L);

			System.out.println("Get should fail with error username/password");
		} catch (HttpStatusCodeException e) {

			System.out.println(e.getStatusCode().equals(HttpStatus.UNAUTHORIZED));
		}
	}
	...
	public static class HttpBasicInterceptor implements ClientHttpRequestInterceptor {
		...
		public ClientHttpResponse intercept(HttpRequest request, byte[] body, ClientHttpRequestExecution execution)
				throws IOException {
			request.getHeaders().set(com.google.common.net.HttpHeaders.AUTHORIZATION, Servlets.encodeHttpBasic(user, password));
